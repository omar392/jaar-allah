        ...
        <div class="container-fluid">
            ...
            <div class="page-title-box">
                <div class="row align-items-center">
                    <div class="col-sm-6">
                        <h4 class="page-title" style="font-family: cairo;">قرارات مجلس الأمة</h4>
                    </div>
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-right">
                            <li class="breadcrumb-item"><a href="{{ route('adminhome') }}">الرئيسية</a></li>
                            <li class="breadcrumb-item active">قرارات مجلس الأمة</li>
                        </ol>
                    </div>
                </div> <!-- end row -->
                <br>
                ...
                <div class="row align-items-center">
                    @if(Auth::guard('admin')->user()->hasPermission('decisions-create'))
                    <div class="text-center">
                        <!-- Large modal -->
                        <button type="button" class="btn btn-primary waves-effect waves-light" data-toggle="modal"
                            data-target=".bs-example-modal-lg"><i class="fas fa-plus-circle"></i>إضافة قرار جديد</button>
                    </div>
                    @endif
                        ...
                        <div class="modal-dialog modal-lg">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title mt-0" id="myLargeModalLabel" style="font-family: cairo;">إضافة قرار جديد</h5>
                                            ...
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-10">
                                                    <input class="form-control" name="url"
                                                        id="example-text-input" placeholder="الرابط">
                                                </div>
                                            </div>
                                            <div class="form-group row">
                                                <label for="example-text-input"
                                    ...
                                    <tbody>
                                        @foreach ($decision as $item)
                                            <tr>
                                                ...
                                                <td>{{ $item->name_ar }}</td>
                                                <td>{{ $item->name_en }}</td>
                                                <td><img src="{{ !empty($item->image) ? url('upload/decision/' . $item->image) : url('upload/no_image.jfif') }}"
                                                ...
                                                <td>
                                                    @if(Auth::guard('admin')->user()->hasPermission('decisions-update'))
                                                    <a href="{{ route('decision.edit', $item->id) }}"><button type="button"
                                                            class="float-left btn btn-info" data-size="sm" title="Edit"><i
                                                                class="fa fa-edit"></i></button></a>
                                                    @endif
                                                    &ensp;
                                                    @if(Auth::guard('admin')->user()->hasPermission('decisions-delete'))
                                                    <form class="float-left ml-1"
                                                        action="{{ route('decision.destroy', $item->id) }}" method="POST">
                                                        @csrf
                                                        @method('delete')
